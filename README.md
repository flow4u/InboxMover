...
**Inbox Mover** is a cross-platform utility designed to process ```Transfer-``` typically found in myDRE ```i:``` and ```z:\inobox```; move/copy files to specific target location into a designated target folder while automatically resolving file conflicts based on your preferences. **Inbox Mover** is optimized to process ZIP files uploaded with FileButler (typically containing a ```receipt.json``` file). It features both a user-friendly Material-inspired graphical interface (GUI) and a Command-Line Interface (CLI) for automation.
