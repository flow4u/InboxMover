...
class InboxMoverCore:
    ...
    def inspect_zip(self, zip_path):
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                if receipt_filename:
                    # If the receipt itself is encrypted we might skip it or fail here
                    # Typically receipt.json inside FileButler zips are not encrypted, 
                    # but if they are, this would raise RuntimeError.
                    try:
    ...
    def process_zip(self, folder_data, config, progress_callback=None, password_callback=None):
        ...
        def extract_zip_file(zip_path):
            ...
            pwd = None # Store provided password for the scope of this zip
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                for i, zinfo in enumerate(file_list):
                    ...
                    # Keep attempting extraction until it succeeds or gets cancelled/fails otherwise
                    while True:
                        try:
                            ...
                            if 'password' in err_str or 'encrypted' in err_str:
                                ...
                                raise e # Re-raise if it's a different RuntimeError
        ...
        try:
            if folder_data.get('has_valid_zip') and folder_data.get('zip_path') and auto_extract:
                ...
                for root, _, files in os.walk(folder_path):
                    for file in files:
                        ...
                        if file.lower().endswith('.zip') and auto_extract:
...
class InboxMoverGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Checkbutton(conflict_col, text="Auto-Extract ZIP files", variable=self.auto_extract_var, style="Card.TCheckbutton").pack(anchor=tk.W, pady=(15, 0))
...
"""
    ...
    def process_current_zip(self):
        ...
        # We need to bridge the password prompt from the background thread safely into the Tkinter main loop
...
def run_cli():
    ...
    parser.add_argument('--no-auto-extract', action='store_true', help='Disable automatic zip extraction and copy the raw zip file')
